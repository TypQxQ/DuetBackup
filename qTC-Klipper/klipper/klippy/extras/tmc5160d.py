...
class TMC5160dCurrentHelper:
    ...
    def _set_globalscaler(self, current):
        logging.info('_set_globalscaler.current: ' + str(current))
        globalscaler = int((current * 256. * math.sqrt(2.)
                            * self.sense_resistor / VREF) + .5)
        logging.info('_set_globalscaler: ' + str(globalscaler))
        globalscaler = max(32, globalscaler)
        if globalscaler >= 256:
            globalscaler = 0
        logging.info('_set_globalscaler Final: ' + str(globalscaler))
    ...
    def _calc_current_bits(self, current):
        ...
        cs = int((current * 256. * 32. * math.sqrt(2.) * self.sense_resistor)
                 / (globalscaler * VREF)
                 - 1. + .5)
        logging.info('sense_resistor: ' + str(self.sense_resistor))
...
class TMC5160d:
    def __init__(self, config):
        ...
        # Register commands
        current_helper = TMC5160dCurrentHelper(config, self.mcu_tmc)
