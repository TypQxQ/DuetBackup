...
class DeltaKinematics:
    def __init__(self, toolhead, config):
        # Setup tower rails
        # self.rails = [stepper.LookupMultiRail(config.getsection('stepper_' + n), False)

        logging.info("Start Delta")

        rail_a = stepper.LookupMultiRail(config.getsection('stepper_a'), False)
        logging.info("rail_a.get_steppers() are: %s", rstr(rail_a.get_steppers()))
        ...
        logging.info("rail_b.get_steppers() are: %s", rstr(rail_b.get_steppers()))
        rail_c = stepper.LookupMultiRail(config.getsection('stepper_c'), need_position_minmax = False, default_position_endstop=a_endstop)
        logging.info("rail_c.get_steppers() are: %s", rstr(rail_c.get_steppers()))
        self.rails = [rail_a, rail_b, rail_c]

        # logging.info("self.rails[0].get_steppers() are: %s", rstr(self.rails[0].get_steppers()))

        logging.info("self.get_steppers() are: %s", rstr(self.get_steppers()))
